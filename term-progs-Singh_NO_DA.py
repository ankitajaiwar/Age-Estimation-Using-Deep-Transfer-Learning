# -*- coding: utf-8 -*-
"""
Created on Tue Dec 11 13:35:12 2018
...
genderte = np.empty((n_train,1), dtype = object)
...
adel = optimizers.Adam(lr = 0.0001)
...
x_l = model.get_layer("flatten").output
...
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.plot(history.history['acc'], label = "Training Accuracy", color = 'g')
plt.plot(history.history['val_acc'], label = "Test Accuracy", color = 'r')
plt.legend()
plt.grid(True)
plt.show()
